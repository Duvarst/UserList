<template>
  <div class="inpfield">
    <div class="btn" @click="showUserAddWindow">Add to user</div>
    <div v-show="flagAddUser">
      <div class="form-group" v-for="(it, i) in userinfo" :key="i">
        <label>
          {{ it.label }}
        </label>
        <input type="text" v-model="it.value" @input="validValue(i, $event)" />
      </div>
      <div>
        <button class="btn btnUserBlock" @click="add">Add</button>
        <button class="btn btnUserBlock" @click="closeUserAddWindow">
          Close
        </button>
