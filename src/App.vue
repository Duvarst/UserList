<template>
  <div id="app">
    <div v-if="!showwind">
      <div>
        <search @inputevent="listEdits($event)"> </search>
      </div>
      <div>
        <adduser
      ...
      <div v-for="(it, i) in userDataArrForDisplay" :key="i">
        <user
          :avatar="it.avatar"
    ...
    <div v-else>
      <user-detali-page @backFromMain="userDetailNone()" :id="id">
      </user-detali-page>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    userSearch(data) {
      ...
      this.userList.forEach((elem) => {
        if (
          elem.first_name.toLowerCase().indexOf(dataSearch) !== -1 ||
          elem.last_name.toLowerCase().indexOf(dataSearch) !== -1
        ) {
          newArr.push(elem);
        }
      });
